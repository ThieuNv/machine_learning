...
def mySONIA(train_X, train_y, test_X, epoch, batch_size, validation,sliding, learning_rate = 0.01, 
    ...
    kmeans = KMeans(n_clusters=120, random_state=0).fit(train_X)
...
#    sorted_list_hu = copy.deepcopy(sorted_list_hu)
    sorted_matrix_Wih = copy.deepcopy(np.transpose(matrix_Wih))
    
    matrix_Who = np.zeros(len(sorted_matrix_Wih[0]))
    bias = 1
