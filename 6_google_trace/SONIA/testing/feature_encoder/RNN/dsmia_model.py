...
# -*- coding: utf-8 -*-
"""
Created on Wed Mar 14 06:13:09 2018

@author: thieunv

*) GradientDescentOptimizer (MinMaxScaler - 2)

    elu / relu/ tanh/ sigmoid ===> 0.34 (nhin figure thi xau) / 0.33(nhin hinh cung xau)/  / (0.58) failed 
    
*) AdamOptimizer 
    
     elu / relu/ tanh/ sigmoid ===> 0.54/ failed / / failed
    
*) AdagradOptimizer   
    
     elu / relu/  tanh/  sigmoid ===> 0.37/ 0.45 /  / failed

*) AdadeltaOptimizer
    
     elu / relu/ tanh/ sigmoid ===> 0.37 / 0.367 / / 0.48
...
class Model(object):
    ...
    def build_bpnn_and_train(self):
        ...
        self.weight_ok, self.bias, self.loss_train = weight, bias, loss_plot
    ...
    def predict_y(self, x, y_prev):
        ...
        for j in range(0, len(self.list_hu)):
            ...
            Dhj = self.alpha * Vhj + self.beta * Zhj
            Xhj.append(np.tanh(Dhj))
        y_temp = np.dot(np.array(Xhj), self.weight_ok ) + self.bias
...
positive_numbers = [0.20] #[0.005, 0.01, 0.025, 0.05, 0.1, 0.15, 0.20]     # [0.1]   
learning_rates = [0.10] #[0.005, 0.01, 0.025, 0.05, 0.10, 0.12, 0.15]   # [0.2]    
hyper_params = [ (0.6, 0.2) ]
sliding_windows = [2] #[ 2, 3, 5]           # [3]  
epochs = [1800] #[100, 250, 500, 1000, 1500, 2000]   # [500]                       
