...
# -*- coding: utf-8 -*-
"""
Created on Thu Dec 28 00:48:17 2017
...
def decrease_list_hidden_unit(list_hu, matrix_weight, percent, sort, how):
    if sort == True:
        matrix_weight = my_sorted(matrix_weight, key_function=square, key_id=1)
        list_hu = my_sorted(list_hu, key_function=take_second_and_square, key_id=2) 
    else:
        new_list_hu = []
        number_hu = int(len(list_hu) * percent)
        if how == 1:    # Cat o dau`
            for i in range(len(list_hu)):
                if i < number_hu:
                    continue
                new_list_hu.append(list_hu[i])
        if how == 2:    # Cat o giua
            temp = int(len(list_hu) - number_hu)
            key = int(len(list_hu) / 2 - temp / 2)
            for i in range(len(list_hu)):
                if i > (int(len(list_hu)/2 - key/2)) and i < (int(len(list_hu)/2 + key/2)):
                    continue
                new_list_hu.append(list_hu[i])
        if how == 3:    # Cat o cuoi
            for i in range(len(list_hu)):
                if i > number_hu:
                    break
                new_list_hu[i] == list_hu[i]
        if how == 4:    # Random
            list_index = []
            for i in range(len(list_hu) - number_hu):
                t = randint(0, len(list_hu)-1 )
                while (t in list_index):
                    t = randint(0, len(list_hu)-1 )
                list_index.append(t)
            for i in range(len(list_hu)):
...
full_path= "/mnt/volume/ggcluster/spark-2.1.1-bin-hadoop2.7/thieunv/machine_learning/6_google_trace/SONIA/testing/mutation/no_update/no_pruning/result/cpu_ram/"
